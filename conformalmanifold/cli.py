...
def _print_fived(geom):
    ...
    if dg["isolated"]:
        ...
        if cu["status"] == "computed":
            fam = f"  ({cu['family']})" if cu["family"] else ""
            print(f"    cubic anomaly (coeff of int c2^3) = {cu['coefficient']}{fam}")
        else:
            print(f"    cubic anomaly: {cu['note']}")
    else:
